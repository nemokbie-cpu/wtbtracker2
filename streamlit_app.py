...
st.title("👟 WTB Tracker – Manual StockX Analysis")
...
def analyze_sales(raw_text, sku, size, listed_price, platform, priority):
    ...
    row = {
        "SKU": sku,
        "Brand": "Auto",
        "Model": "Paste name manually if needed",
        "Colorway": "Paste if needed",
        "Size": size,
...
with st.expander("➕ Add New WTB Entry", expanded=True):
    ...
    with col4:
        raw_sales = st.text_area("Paste Raw StockX Sales Data", height=150)
...
for tab, p in zip([tab1, tab2, tab3], platforms):
    with tab:
        ...
        styled = df.style.apply(color_priority, axis=1)
...
# ─── HIGH BIDS SECTION ───────────────────────────────────────────
st.header("🔥 High Bids / Good ROI Auto-Flags")
high_rows = []
for p in platforms:
    df = st.session_state.tables[p]
    if not df.empty:
...
if high_rows:
    ...
    st.info("No items meet high-bid/ROI flags yet")
...
if st.button("Export All Tables to CSV"):
    all_data = pd.concat(st.session_state.tables.values(), ignore_index=True)
    st.download_button("Download CSV", all_data.to_csv(index=False), "wtb_tracker_export.csv")

st.caption("Manual entry & analysis • Paste StockX sales data • Priority colors applied")
